...
- checked Nat arithmetic including total / and %, bounded Nat equality/order
  comparison, Bool logic, and checked Bool/ordering if conditions;
- lambdas and lets;
...
2. Continue verified primitive/equality/comparison semantics beyond the landed
   Nat +,-,*,/,%, ==/!= and Bool !/&&/|| paths only where Lean-compatible
   meaning is explicit.
