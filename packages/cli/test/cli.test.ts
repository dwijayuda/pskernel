...

{
  ...
  for(const item of sources){
    const directory=await mkdtemp(join(tmpdir(),'proofscript-sh2-array-'));
    try{
      await mkdir(join(directory,'src'),{recursive:true});
      await writeFile(
        join(directory,'psconfig.json'),
...
console.log('ok - psc SH2 dual-source canonical Array runtime');
...

{
  ...
  try{
    ...
    await writeFile(
      join(directory,'src','main.ps'),
      'import Data\nfunction main(x : Nat) : Nat := foundationIdentity(x);\n',
      'utf8',
    );
    await writeFile(
      join(directory,'src','Data.ps'),
