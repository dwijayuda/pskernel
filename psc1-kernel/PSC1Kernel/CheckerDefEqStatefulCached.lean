...
import PSC1Kernel.CheckerReductionCached

namespace PSC1Kernel

namespace StatefulDefEqCached
...
partial def isProp
  ...
  let (reduced, state2) ← StatefulReductionCached.whnf defeq ctx state1 type
  match reduced with
...
partial def unitLike
  ...
  let (tType, state2) ← StatefulReductionCached.whnf defeq ctx state1 tType0
...
partial def isDefEq
  ...
  if !a.hasFVar || ctx.eagerReduce then
    ...
    | .const name levels =>
        if levels.length == 0 && Name.eq name kernelBoolTrueName then
          let (reduced, next) ← StatefulReductionCached.whnf isDefEq ctx current a
          current := next
          match reduced with
  ...
  let (aCore, state2) ← StatefulReductionCached.whnfCore isDefEq ctx current a false true
  let (bCore, state3) ← StatefulReductionCached.whnfCore isDefEq ctx state2 b false true
  ...
  let (aFull, state9) ← StatefulReductionCached.whnfCore isDefEq ctx state8 aDelta false false
  let (bFull, state10) ← StatefulReductionCached.whnfCore isDefEq ctx state9 bDelta false false
  ...
  | .lam _ _ _ _, other =>
      ...
      let (otherType, next2) ← StatefulReductionCached.whnf isDefEq ctx next1 otherType0
      finalState := next2
