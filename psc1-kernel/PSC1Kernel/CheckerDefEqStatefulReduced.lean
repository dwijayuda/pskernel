...
/--
Lazy-delta unfolding must normalize an unfolded body through the same recursive
stateful reducer as the caller. This closes the remaining recursor/Quot state
escape without changing Nat reduction's low-memory behavior.
-/
...
/--
Lean-4.34 lazy delta with stateful delta/projection normalization. Nat/native
reduction deliberately remains on the prior low-memory path; this function does
not retain WHNF state for every Nat operand.
-/
