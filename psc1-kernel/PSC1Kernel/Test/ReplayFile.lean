...
def replayPushEqAppendTheorem
  ...
  let shared := ShareCommon.shareCommon' (value, type)
  let proof := shared.1
  let theoremType := shared.2
  let info : TheoremInfo := {
    base := { name := name, levelParams := levelParams, type := theoremType }
    value := proof
  }
  ...
  let compared := ShareCommon.shareCommon' (valueType, info.base.type)
  IO.println "PSC1 Lean theorem PHASE final-defeq-begin"
  let eq ← liftReplayResult "<diagnostic>" 0
    (isDefEq ctx compared.1 compared.2)
