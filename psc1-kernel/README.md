...
The direct Lean 4.34 oracle now covers the kernel-level forms exercised by
Lean's nested-inductive tests rather than only one `Box Tree` example:

- shared parameters and universe-polymorphic nested families,
- repeated nesting analogous to `List (List Tree)`,
- indexed outer type formers analogous to `Vec Tree n`,
- ordinary outer mutual families,
- restored main/auxiliary recursor metadata and reduction,
- fixed-parameter uniformity and reserved `_nested` fail-closed cases.

`NestedInductive.lean` has no capability branch labeled unsupported; its
remaining errors are structural or metadata validation failures. K5 therefore
uses "semantic baseline complete" in the same bounded sense as K2/K8: it does
not imply full Lean-environment replay or formal equivalence.
