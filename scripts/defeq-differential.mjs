...
const cases=[
 ...
 {name:'native-reduce-nat-kernel-provable',expected:true,lean:'def NativeReduceNatV := 100000000000 + 100000000000\nexample : Lean.reduceNat NativeReduceNatV = 200000000000 := rfl',ts:()=>{const n=N('Diff.NativeReduceNatV');if(!env.has(n))env.add({kind:'definition',name:n,levelParams:[],type:Nat(),value:add(natLit(100000000000n),natLit(100000000000n)),hints:{kind:'regular',height:1n},safety:'safe'});return {tc:new TypeChecker(env),lhs:app(constant(N('Lean.reduceNat')),constant(n)),rhs:natLit(200000000000n)};}},
 {name:'native-reduce-bool-kernel-provable',expected:true,lean:'def NativeReduceBoolV : Bool := 200000000001 <= 20000000000\nexample : Lean.reduceBool NativeReduceBoolV = false := rfl',ts:()=>{const n=N('Diff.NativeReduceBoolV');if(!env.has(n))env.add({kind:'definition',name:n,levelParams:[],type:Bool(),value:mkAppN(constant(N('Nat.ble')),[natLit(200000000001n),natLit(20000000000n)]),hints:{kind:'regular',height:1n},safety:'safe'});return {tc:new TypeChecker(env),lhs:app(constant(N('Lean.reduceBool')),constant(n)),rhs:constant(N('Bool.false'))};}},
