...
const cases=[
 ...
 {name:'nat-succ-literal',expected:true,lean:'example : (Nat.succ 2) = 3 := rfl',ts:()=>({tc:new TypeChecker(env),lhs:app(constant(N('Nat.succ')),natLit(2)),rhs:natLit(3)})},safety:'safe'});const nativeEvaluator={evaluate(_env,request){return request.kind==='bool'&&request.constant===n?{kind:'bool',value:false}:null;}};return {tc:new TypeChecker(env,undefined,undefined,undefined,'safe',undefined,false,nativeEvaluator),lhs:app(constant(N('Lean.reduceBool')),constant(n)),rhs:constant(N('Bool.false'))};}},
