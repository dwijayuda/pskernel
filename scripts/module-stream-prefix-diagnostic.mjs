...
const stopAfterShards=Number(process.env.PSKERNEL_DIAGNOSTIC_STOP_AFTER_SHARDS??'2400');
if(!Number.isSafeInteger(stopAfterShards)||stopAfterShards<1)throw new Error('invalid diagnostic stop shard');
...
let fatal=null,prefixStopped=false;
try{
...
if(!prefixStopped&&code!==0)throw new Error(`Lean exporter exited ${code}: ${stderr}`);
if(!header)throw new Error('missing environment header');
if(prefixStopped){
  const finalMem=process.memoryUsage();
  maxRssMiB=Math.max(maxRssMiB,finalMem.rss/1048576);maxHeapMiB=Math.max(maxHeapMiB,finalMem.heapUsed/1048576);
  console.log(JSON.stringify({
    ...
    leanGitHash,
    module:moduleName,
    plannedShards:Number(header.plannedShards??0),
    stopAfterShards,
    shards,
    records:totalLines,
