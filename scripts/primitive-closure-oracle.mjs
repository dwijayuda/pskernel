...
const expected={lines:15780,names:2482,levels:18,expressions:12931,declarations:348};
for(const [k,v] of Object.entries(expected)) if(stats[k]!==v) throw new Error(`primitive closure ${k} drift: got ${stats[k]}, expected ${v}`);
if(replay.env.entries().length!==419) throw new Error(`primitive closure constant count drift: got ${replay.env.entries().length}, expected 419`);
for(const n of [N.NatAdd,N.NatSub,N.NatPred,N.NatBeq,N.NatBle,N.NatMod,N.NatDiv,N.NatGcd,N.NatBitwise,N.NatLand,N.NatLor,N.NatXor,N.NatShiftLeft,N.NatShiftRight,N.NatBitwiseUnaryProof1]){
