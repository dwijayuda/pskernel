...
import { nameEq, nameFromDotted } from '../core/name.js';
...
export function checkEqShape(env:Environment):void{
 ...
 const r=env.find(eq.ctors[0]!);if(!r||r.kind!=='constructor'||r.levelParams.length!==1||!nameEq(r.levelParams[0]!,eqParam)||!exprEq(r.type,expectedReflType(eqParam)))throw new KernelError('failed to initialize Quot: Eq.refl has unexpected type');
}
