...
import { reduceNative } from './reduction/native.js';
...
export class TypeChecker {
  ...
  whnf(e:Expr):Expr{return this.rec(()=>{
    ...
    for(let fuel=0;fuel<100000;fuel++){
      ...
      const native=reduceNative(this.env,x,z=>this.whnf(z));if(native){x=native;continue;}
  ...
  isDefEq(a:Expr,b:Expr):boolean{return this.rec(()=>{
    ...
    for(let i=0;i<512;i++){
      ...
      const nx=reduceNative(this.env,x,z=>this.whnf(z));if(nx)return this.isDefEq(nx,y);const ny=reduceNative(this.env,y,z=>this.whnf(z));if(ny)return this.isDefEq(x,ny);
